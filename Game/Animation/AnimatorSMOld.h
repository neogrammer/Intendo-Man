...
namespace game::anim
{
    struct AnimContext
    {
        ...
        bool wantDash{ false }; // true while dash/airdash is active
    ...
    struct AnimProfile
    {
        ...
        void ValidateAgainst(Actor const& a)
        {
            ...
            check(idleCharge);
            check(runCharge);
            check(jumpRiseCharge);
            check(jumpPeakCharge);
            check(fallCharge);

            check(wallGrabCharge);
            check(wallSlideCharge);
            check(wallKickCharge);
    ...
    public:
        ...
        void Tick(Actor& a, float dt, AnimContext const& ctx, AnimProfile const& profile)
        {
            ...
            // --- Air logic (AirDash / Wall / JumpRise / JumpPeak / Fall)
            if (!ctx.grounded)
            {
                // Air dash takes priority if it exists
                if (ctx.wantDash)
                {
                    if (auto* key = profile.Resolve(Locomotion::AirDash, ov); key && a.hasClip(*key))
                    {
                    ...
                    // Fallback: if you don't define an AirDash clip, reuse Dash in air.
                    if (auto* key = profile.Resolve(Locomotion::Dash, ov); key && a.hasClip(*key))
                    {
                        ApplyDesired(a, profile, Locomotion::Dash, ov);

                        if (Locomotion::Dash != m_loco || ov != m_overlay)
                        {
                            m_timeInState = 0.0f;
                            m_loco = Locomotion::Dash;
                            m_overlay = ov;
                        }
                        else
                        {
