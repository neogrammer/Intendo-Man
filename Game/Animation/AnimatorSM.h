...
namespace game::anim
{
    struct AnimContext
    {
        ...
        // air dash (computed by PlayState)
    ...
    struct AnimProfile
    {
        ...
        std::optional<std::wstring> airDash;
        ...
        std::optional<std::wstring> airDashShoot;
        ...
        std::optional<std::wstring> airDashCharge;
        ...
        void ValidateAgainst(Actor const& a)
        {
            ...
            check(idle); check(run); check(jumpRise); check(jumpPeak); check(fall); check(airDash); check(land);
            ...
            check(fallShoot); check(airDashShoot); check(landShoot);
            ...
            check(fallCharge); check(airDashCharge);
        ...
        std::wstring const* Resolve(Locomotion loco, Overlay ov) const noexcept
        {
            ...
            if (ov == Overlay::Shoot)
            {
                ...
                if (loco == Locomotion::AirDash)  if (auto p = ptr(airDashShoot))  return p;
            ...
            else if (ov == Overlay::Charge)
            {
                ...
                if (loco == Locomotion::AirDash)  if (auto p = ptr(airDashCharge))  return p;
            ...
            if (loco == Locomotion::AirDash)  if (auto p = ptr(airDash))  return p;
    ...
    public:
        ...
        void Tick(Actor& a, float dt, AnimContext const& ctx, AnimProfile const& profile)
        {
            ...
            if (!ctx.grounded)
            {
                ...
                // Air dash overrides everything while active.
                if (ctx.airDashing)
                {
                    ApplyDesired(a, profile, Locomotion::AirDash, ov);

                    if (Locomotion::AirDash != m_loco || ov != m_overlay)
                    {
                        m_timeInState = 0.0f;
                        m_loco = Locomotion::AirDash;
                        m_overlay = ov;
                    }
                    else
                    {
    ...
    private:
        ...
        static bool IsAirDashVariant(std::wstring const& k, AnimProfile const& p) noexcept
        {
            return Eq(k, p.airDash) || Eq(k, p.airDashShoot) || Eq(k, p.airDashCharge);
        }
        ...
        static bool SameSyncGroup(std::wstring const& a, std::wstring const& b, AnimProfile const& p) noexcept
        {
            ...
            if (IsAirDashVariant(a, p) && IsAirDashVariant(b, p)) return true;
