...
loop_delay = 0
    ...
    void PlayState::update(float dt_)
    {
        ...
        if (actMap && player)
        {
            ...
            // Chef's kiss: apex hang (reduce gravity near the top)
            ...
            // Dash / Hit / Death / Invuln
            constexpr int   maxHealth = 5;

            constexpr float dashSpeed = 720.0f;     // px/s
            constexpr float dashDuration = 0.18f;   // seconds
            constexpr float dashCooldown = 0.10f;   // seconds

            constexpr float hitStun = 0.25f;        // seconds movement lock
            constexpr float hitKnockX = 260.0f;     // px/s
            constexpr float hitKnockY = 380.0f;     // px/s (up is negative)

            constexpr float invulnTime = 1.00f;     // seconds after hit
            constexpr float invulnBlinkPeriod = 0.08f; // seconds per visible/invisible phase
            ...
            // Hit knockback horizontal velocity
            static float hitVelX = 0.0f;

            // Dash state (used for BOTH ground-dash and air-dash)
            static float dashTimer = 0.0f;
            static float dashCooldownTimer = 0.0f;
            static float dashDir = 1.0f; // -1 left, +1 right

            // Air dash: allow ONCE per airtime (resets when grounded; also resets on wall jump)
            ...
            static float invulnFlashPhase = 0.0f;
            ...
            bool dashPressed = actMap->Pressed(engine::Action::Dash);

            // Debug: hit test
            bool debugHitPressed = actMap->Pressed(engine::Action::Fire);
            ...
            if (wasGrounded)
            {
                ...
                // Reset air-dash availability when we have feet on the ground.
                airDashUsed = false;
            }

            bool controlLocked = dead || (hitStunTimer > 0.0f);

            // Debug: apply a hit (respects invuln)
            if (debugHitPressed && !dead && invulnTimer <= 0.0f)
            {
                health -= 1;
                hitStunTimer = hitStun;
                invulnTimer = invulnTime;
                invulnFlashPhase = 0.0f;
                ...
                float knockDir = player->IsFacingRight() ? -1.0f : 1.0f;
                player->inAir();
                velY = -hitKnockY;
                hitVelX = knockDir * hitKnockX;
            ...
            // While in hitstun (or dead), we still apply physics but ignore inputs.
            if (controlLocked)
            {
                dashPressed = false;
                jumpPressed = false;
            ...
            // 1px wall probes (like the under-probe, but on the sides)
            ...
            // Probe at the start of the frame (before movement)
            ...
            winrt::Windows::Foundation::Rect leftProbePre{ posPre.x - kWallProbeW, posPre.y + kWallProbeInsetY, kWallProbeW, probeHPre };
            winrt::Windows::Foundation::Rect rightProbePre{ posPre.x + sizePre.x, posPre.y + kWallProbeInsetY, kWallProbeW, probeHPre };
            ...
            // Jump press routing:
            // - if touching a wall and pressing into it -> wall jump
            // - else if grounded/coyote -> normal jump
            // - else if falling -> buffer (pre-landing)
            if (jumpPressed)
            {
                if (pressingIntoWallPre)
                {
                    ...
                    // Wall jump gives you your air-dash back (feels very MegaMan)
                    airDashUsed = false;
            ...
            // --- DASH (ground + air)
            if (!controlLocked && dashPressed && dashTimer <= 0.0f && dashCooldownTimer <= 0.0f)
            {
                // If you're on a wall and pressing into it, default dash AWAY from it.
                if (!wasGrounded && pressingIntoWallPre)
                {
                    dashDir = touchWallLeftPre ? 1.0f : -1.0f;
                }
                else
                {
                    // Otherwise: input, else facing
                    if (move.x < -0.20f) dashDir = -1.0f;
                    else if (move.x > 0.20f) dashDir = 1.0f;
                    else dashDir = player->IsFacingRight() ? 1.0f : -1.0f;
                }

                // Start ground dash any time you're grounded.
                // Start air dash only once per airtime.
                if (wasGrounded)
                {
                ...
                else if (!airDashUsed)
                {
                    ...
                    dashCooldownTimer = dashCooldown;
                    airDashUsed = true;
                    wallJumpLockTimer = 0.0f; // let air dash override wall-jump kick if you do it right after
                    Cfg::PlaySfx(L"blip");
                }
            }
            ...
            // Wall slide clamp (so it applies immediately this frame)
            bool wallSlidingPre = (!wasGrounded) && pressingIntoWallPre && (velY > 0.0f) && (dashTimer <= 0.0f) && !controlLocked;
            ...
            {
                // During wall-jump lock, force horizontal kick-off
                xVel = wallJumpVelX;
            }
            else if (dashTimer > 0.0f)
            {
                // Dash overrides input
                xVel = dashDir * dashSpeed;
            }
            else if (wallSlidingPre)
            {
                // While sliding, don't keep shoving into the wall every frame
                xVel = 0.0f;
            }
            ...
            // If we touched ground at any point this frame, reset airdash availability.
            if (nowGrounded)
            {
                airDashUsed = false;
            }
            ...
            winrt::Windows::Foundation::Rect leftProbe{ posPost.x - kWallProbeW, posPost.y + kWallProbeInsetY, kWallProbeW, probeHPost };
            winrt::Windows::Foundation::Rect rightProbe{ posPost.x + sizePost.x, posPost.y + kWallProbeInsetY, kWallProbeW, probeHPost };

            bool touchWallLeft = (!nowGrounded) && ProbeSolid(leftProbe);
            bool touchWallRight = (!nowGrounded) && ProbeSolid(rightProbe);

            bool wallSliding = (!nowGrounded) &&
                (velY > 0.0f) &&
                ((touchWallLeft && pressLeft) || (touchWallRight && pressRight));

            if (dashTimer > 0.0f)
            {
                // During dash, don't advertise wall-slide to the animator.
                wallSliding = false;
            }
            ...
            // --- Invulnerability flashing (alpha blink)
            if (!dead && invulnTimer > 0.0f)
            {
                invulnFlashPhase += dt_;
                const float cycle = invulnBlinkPeriod * 2.0f;
                if (invulnFlashPhase >= cycle)
                    invulnFlashPhase -= cycle;

                const bool visible = (invulnFlashPhase < invulnBlinkPeriod);
                const float a = visible ? 1.0f : 0.20f;

                player->SetTint(winrt::Windows::Foundation::Numerics::float4{ 1.0f, 1.0f, 1.0f, a });
            }
            else
            {
                invulnFlashPhase = 0.0f;
                player->SetTint(winrt::Windows::Foundation::Numerics::float4{ 1.0f, 1.0f, 1.0f, 1.0f });
            }
            ...
            if (auto* p = dynamic_cast<Player*>(player.get()))
            {
                ...
                animCtx.wantDash = (!controlLocked) && (dashTimer > 0.0f);
                ...
                animCtx.gotHit = (hitStunTimer > 0.0f) && !dead;
                animCtx.dead = dead;
